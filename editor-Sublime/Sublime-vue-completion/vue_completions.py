...
def remove_html_css_completions():
    # 移除Sublime自带的html提示
    for completer in ["HTML.html_completions.HtmlTagCompletions"]:
...
class TagCompletions(sublime_plugin.EventListener):
    ...
    def on_query_completions(self, view, prefix, locations):
        ...
        # 仅针对wxml有效
        if not view.match_selector(locations[0], "text.html.vue"):
            return []

        # 检测当前作用域是否在标签内
        is_inside_tag = view.match_selector(locations[0],
                "text.html.vue meta.tag - text.html.vue punctuation.definition.tag.begin")
    ...
    def get_attribute_completions(self, view, locations, prefix, ch):
        ...
        if ch == '"':
         ...
         # 属性名称的completion列表
         # 如果在字符串作用下则不输出
        elif not view.match_selector(locations[0], "text.html.vue string.quoted"):
            attr_data = {}
            private_attr = setting_cache.tag_data.get(tag, {}).get('attrs', [])
            attr_list = []
            attr_list.extend(private_attr)
            ...
            # 属性值的动态绑定
            if attr == 'v-bind:' or attr == ':':
                private_attr_list = [(item.get('name', '')) for item in private_attr]
                private_attr_list.extend(setting_cache.html_attributes)
                return [(name + '\tTag', name + '="$1"') for name in private_attr_list]
