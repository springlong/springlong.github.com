...
(function(window){
    ...
    jsApp.init = function(selector)
    {
        ...
        //如果为数组
        jsApp.isArrayLike(selector) && makeIn.call(_self, selector);
    }
    ...
    jsApp(function()
    {
        var tags, i, len, ele, head, children;

        //使IE浏览器兼容HTML5标签
        /*----------------------------------
         *说明：截止2012-09-03，Firefox、Chrome、Safari、Opera、IE9等高级浏览器均已支持基本的HTML5标签，但是在IE8及更低版本的IE浏览器中无法使用它们。
         *解决办法：在IE中，只需要通过document.createElement()方法创建一个未知的HTML元素，就可以正常使用IE本身未支持的标签（创建后标签默认为行内元素）。
        ...
        //用于兼容IE6、7中的querySelectorAll方法，并为IE6~8中兼容HTML5设置块状元素
        if(!document.querySelectorAll || !document.addEventListener)
        {
            ele = document.createElement("style");
            ele.id = "jsApp_selectorStyle";
            ele.type = "text/css";
            ele.styleSheet.cssText = "header,footer,aside,article,section,hgroup,nav,menu,canvas,details,figure,figcaption,audio,video{display:block;}";
           
            head = document.getElementsByTagName("head")[0];
            children = head.children;
            children.length > 0 ? (head.insertBefore(ele, children[0])) : (head.appendChild(ele));
        }
    });
