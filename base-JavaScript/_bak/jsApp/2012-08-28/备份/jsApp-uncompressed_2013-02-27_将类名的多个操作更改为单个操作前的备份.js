...
(function(window){
	...
	jsApp.fn = jsApp.prototype = {
		...
		attr:function(attr, value)
		{
			...
			if(value)
			{
				attr === "class" ? ele.className = value : ele.setAttribute(attr, value);
				return this;
			}
			return attr === "class" ? ele.className : ele.getAttribute(attr);
		},
		...
		{
			return this.node.getAttribute(attr) !== null;
		},
		...
		addClass:function(name)
		{
			...
			{
				addStr = name[i];
				!this.hasClass(addStr) && (ele.className += " " + addStr);
			}
			return this;
		},
		...
		{
			return (" " + this.node.className + " ").indexOf(" " + name + " ") !== -1;
		},
		...
		toggleClass:function(name)
		{
			var _this = this,
				...
				name = name.split(" "),
				len = name.length,
				isExist = _this.hasClass(name[0]),
				toggleStr, i;

			for(i = 0; i < len; i++)
			{
				toggleStr = name[i];
				isExist ? _this.removeClass(toggleStr) : _this.addClass(toggleStr);
			}
			return this;
		},
	...
	jsApp.extend({
		...
		*/
		 ...
		 *4. char：（类型：string，必备）用来进行填充的字符，如果是以&开头且长度大于1的字符串作为特殊HTML编码字符来使用，如果不是以&开头且长度大于1的字符串将截取第一个字符；
		 *=================================================================================*/
		padStr: function(str, dir, len, char)
		{
		    ...
		    if(strLen < len)
		    {
		        char = (char.charAt(0) === "&" && char.length > 1) ? char : char.charAt(0);
		        len = len - strLen;
