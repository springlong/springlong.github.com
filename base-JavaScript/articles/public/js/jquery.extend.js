 ...
 * @attention   该脚本文件请使用YUI压缩，使用Uglify压缩将导致脚本异常！
 */
...
jQuery.fn.scrollWith = function(params)
{
    ...
    return this.each(function(index, ele)
    {
        ...
        $win.bind("load", function()
        {
            ...
            //如果标元素的高度大于文档可视区高度时，同样需要做相关兼容处理
...
/**
 * <<幻灯片（同样适用于图层滚动）>>，引用：jQuery.getDivision()
 * @param  {Object} [params] 参数配置
 * @param  {String} params.contents 用来表示内容列表的查询字符串，默认为：>.content>ul>li
 * @param  {String} params.tags 用来表示标签列表的查询字符串，默认为：>.tag>ul>li
 * @param  {String} params.trigger 触发方式，默认为“mouseover”（鼠标悬浮切换），也可设置为"click"（点击切换）
 * @param  {Boolean} params.auto 是否自动切换，默认为false
 * @param  {Number} params.delay 自动切换时的时间间隔，默认为5000（单位：毫秒）
 * @param  {Number} params.duration 具有切换效果时的效果持续时间，默认为400（单位：毫秒）
 * @param  {Number} params.easing 进行动画效果时的缓动算法，默认为swing
 * @param  {String} params.effect 切换方式，默认无效果，"none"-无效果，"fade"-淡入淡出，"fadeIn"-仅淡入效果，scrollx"-垂直滚动，"scrolly"-水平滚动，"accordition"-手风琴效果，"widthClass"-通过内容项的show类名来控制显示状态。
 * @param  {Number} params.scrollLen 每次切换将滚动的单位长度（按单张图片的宽度或高度计算），默认为1
 * @param  {Number} params.cur 默认显示的幻灯片的索引，默认为0
 * @param  {Number} params.appends 特殊无缝结构的附加量，默认为0，仅针对无缝结构有效
 * @param  {Boolean} params.keepTags 是否保留标签项的内容，默认为false
 * @param  {Boolean} params.accordition 手风琴效果的最低展示区域宽度或高度，默认为0，当设置该参数大于0时则表示为手风琴的无标题项结构
 * @param  {Boolean} params.hoverDelay 鼠标悬浮切换时的等待延时，默认为50毫秒。
 * @param  {Boolean} params.seamless 是否实现无缝结构，默认为false
 * @param  {Boolean} params.text 针对内容项文本标题的操作，默认为“none”——表示没有相关操作，“show”——表示普通隐藏显示操作，“slide”——表示对文本标题项做滑出效果
 * @param  {Boolean} params.doMarquee 是否执行无缝滚动效果，默认为false
 * @param  {Boolean} params.speed 用来设置无缝滚动执行时的速度，“fase”-快速、“normal”-正常、“slow”-缓慢
 * @param  {Boolean} params.lazyload 是否启用图片来加载功能，默认为false-不启用（无缝滚动效果不支持图片懒加载，同时也不建议将图片列表做成无缝滚动效果）
 * @param  {Boolean} params.useDefault 是否使用默认的结构处理，即只提供内容列表，样式以及其他的结构布局均由脚本完成，默认设置为false
 * @param  {String} params.className 使用默认结构处理时是否使用自定义的样式，该参数默认为空字符串——表示默认使用该组件自带的类名为“jsModuleSlide”的样式表现
 * @param  {Boolean} params.showTags 仅当使用useDefault时有效，用来设置是否需要显示标签项，默认为true
 * @param  {Function} params.startFun 切换之前执行的回调函数——function(cur, old){}，cur参数表示当前需要显示的索引项，old参数表示前一个显示的索引项，this指向一个新的对象——{ele: 外围容器的jQuery对象, contents: 内容列表的jQuery对象, tags: 标签项列表的jQuery对象}
 * @param  {Function} params.endFun 切换之后执行的回调函数——function(cur, old){}，cur参数表示当前需要显示的索引项，old参数表示前一个显示的索引项，this指向一个新的对象——{ele: 外围容器的jQuery对象, contents: 内容列表的jQuery对象, tags: 标签项列表的jQuery对象}
 * @return {jQuery}        原jQuery对象
 */
jQuery.fn.slide = function(params)
{
    //默认参数与传递参数合并
    ...
    config.effect.indexOf("s") !== 0 && (config.seamless = false);      //无缝结构仅针对滚动效果有效
    config.doMarquee && (config.seamless = true);                       //无缝滚动效果必须是无缝结构
    config.effect === "fade" && params.easing === undefined && (config.easing = "easeOutCirc");    //淡入淡出效果的默认缓动算法
    ...
    return this.each(function(index, ele)
    {
        ...
        {
            //添加结构布局
            $ele.addClass(config.className || "jsModuleSlide")
                .children().css({overflow: "hidden", display: "block", width: $ele.width(), height: $ele.height() })
                .wrapAll('<div class="content"></div>');

            config.showTags && $ele.append('<div class="tag"><ul><li></li></ul></div>');

            //添加默认样式
            config.className === "" && jQuery("#jsModuleSlide").length === 0 && jQuery("head").append("<style id='jsModuleSlide'>.jsModuleSlide{position:relative;}.jsModuleSlide .content{position:relative;overflow:hidden;width:100%;height:100%;}.jsModuleSlide .content img{border:none;}.jsModuleSlide .tag{position:absolute;left:0;bottom:0;width:100%;font-size:0;letter-spacing:-8px;text-align:right;}.jsModuleSlide .tag ul{*zoom:1;padding:4px 10px;opacity:0.7;filter:alpha(opacity=70);}.jsModuleSlide .tag li{display:inline-block;*zoom:1;*display:inline;width:18px;height:18px;border:1px solid #ccc;margin-left:3px;font-size:12px;line-height:18px;font-family:Verdana;letter-spacing:normal;text-align:center;color:#000;background-color:#fff;cursor:pointer;}.jsModuleSlide .tag li.sele{border-color:#1188C0;color:#fff;background-color:#1072AA;}</style>");
        }

        //核心DOM操作对象
        var $wrap = $ele.find(">.content"),                 //内容容器
            $contents = $wrap.children(),                   //内容列表
            $tagWrap = $ele.find(">.tag"),                  //标签外围容器
            $tags = $tagWrap.children(),                    //标签列表
            $posCur = $ele.find(">.pos>span.pos-cur"),      //显示当前页的容器
            $posTotal = $ele.find(">.pos>span.pos-total"),  //显示总页数的容器
            $btnPrev = $ele.find(">.btn>.btnPrev"),         //向前按钮
            $btnNext = $ele.find(">.btn>.btnNext");         //向后按钮

        //确保正确的内容列表和标签列表
        $contents = $wrap.length === 1 && $contents.length > 1 ? $contents : $ele.find(config.contents);
        $tags = $tags.length > 1 ? $tags : $ele.find(config.tags);

        //处理所需相关变量
        var $contentsParent = $contents.eq(0).parent(), //内容列表的父节点
            $text = $wrap.find(".text"),                //内容项的指定标题文本
            $tagsParent = $tags.eq(0).parent(),         //标签列表的父节点
            $links = $tags.children("a"),               //标签项的链接集合
            effect = config.effect,                     //参数变量简写
            scrollLen = config.scrollLen,               //参数变量简写
            seamless = config.seamless,                 //参数变量简写
            appends = config.appends,                   //参数变量简写
            duration = config.duration,                 //参数变量简写
            trigger = config.trigger,                   //参数变量简写
            auto = config.auto,                         //参数变量简写
            easing = config.easing,                     //参数变量简写
            accordition = config.accordition,           //参数变量简写
            endFun = config.endFun,                     //参数变量简写
            amount = $contents.length,                  //内容列表的总数
            cansee = 1,                                 //可视图片的张数
            scrollNums = scrollLen,                     //每次切换将滚动几张图片，默认与每次切换将滚动的单位长度一致
            pages,                                      //所拥有的总页数
            scrollBase = effect === "scrollx" ? $contents.outerWidth(true) : $contents.outerHeight(true),   //滚动效果时每单位的距离
            scrollValue = scrollBase * scrollLen,       //实际滚动效果时应该移动的距离
            oldIndex = config.cur,                      //现有显示内容所在的页数
            minIndex = 0,                               //内容项的最小索引
            isMouseente = trigger === "mouseenter", //是否是鼠标悬浮触发
            isPaused = false,                           //自动切换时是否处于暂停状态
            stopDelay = 0, stopAuto = 0, stopFocus = 0, //setInterval和setTime的id
            lastNums, scrollMax,                        //“scrollx”效果所需的处理变量
            tagScroll, tagCansee, tagLength, tagDistance = 0, tagScrollBox,   //标签项是否需要滚动的状态标识
            marqueeNext = true,                         //无缝滚动的方向是否为：显示下一张图片
            speed,                                      //保存无缝滚动的速度值
            direct, doStyle,                            //scrollx和scrolly效果所需变量
            isLocked = false,                           //是否处于锁定状态
            acc_direct, acc_value,                      //手风琴效果所需变量
            i = 0, str = "";

        //校正部分变量
        $tagWrap.length === 0 && ($tagWrap = $tagsParent);

        //正确计算可视图片的张数以及每次切换将滚动几张图片
        if($wrap.length === 1)
        {
            var cols = jQuery.getDivision($wrap.width(), $contents.outerWidth(true)),
                rows = jQuery.getDivision($wrap.height(), $contents.outerHeight(true));

            cansee = rows * cols; //可视图片的张数
            scrollNums = effect === "scrolly" ? scrollLen * cols : scrollLen;  //每次切换将滚动几张图片
        }
        pages = jQuery.getDivision(amount, scrollNums);

        //如果内容列表的数量没有超过可视的图片张数，则不执行后续操作
        if(amount <= cansee)
        {
            $ele.children(".tag, .pos, .btn").remove();
            return;
        }
        ...
        {
            //如果无缝结构的滑动效果中，每单位移动量跟总量不构成倍数的情况下，将被重置为非无缝结构
            if(seamless && amount % scrollNums !== 0)
            {
                seamless = false;
            }
            ...
            if(effect.indexOf("s") === 0)
            {
                ...
                {
                    $contentsParent = $contents.wrapAll("<div style='position:relative;'></div>").eq(0).parent();
                }
            }
            ...
            if($tags.length)
            {
                ...
                if(!config.keepTags)
                {
                    ...
                    $tags = $tagsParent.html(str).children();
                }
                ...
                $tags.bind(trigger, function(e)
                {
                    ...
                    }, isMouseente ? config.hoverDelay : 0); //鼠标悬浮延时处理
                });
                ...
                $links.focus(function(e)
                {
                    ...
                    stopFocus = setTimeout(function()
                    {
                        doShow(index + minIndex);
                        doPause();
                    }, 100); //延时处理是因为在Firefox和IE浏览器中，鼠标点击会先触发focus事件，再触发链接的点击事件，造成链接标签项点击切换时会直接打开url操作
                });
                auto && $links.blur(doPlay);

                //如果为点击切换，则标签项的链接仅在选中状态、或者有功能按键的情况下有效
                if($links.length > 0 && trigger === "click")
                {
                    $links.click(function(e)
                    {
                        clearTimeout(stopFocus);
                        !(this.on || e.ctrlKey || e.shiftKey || e.altkey) && e.preventDefault();
                    });
                }

                //判断标签项是否需要滚动切换（确保标签项在一个容器内）
                if($tags.eq(0).next().is($tags.eq(1)))
                {
                    if($tagWrap.width() < $tagWrap[0].scrollWidth)
                    {
                        tagLength = $tags.outerWidth(true);
                        tagCansee = jQuery.getDivision($tagWrap.width(), tagLength);
                        tagScroll = "scrollLeft";
                        tagScrollBox = $tagWrap;
                    }
                    else if($tagsParent.height() < $tagsParent[0].scrollHeight)
                    {
                        tagLength = $tags.outerHeight(true);
                        tagCansee = jQuery.getDivision($tagsParent.height(), tagLength);
                        tagScroll = "scrollTop";
                        tagScrollBox = $tagsParent;
                    }
                }
            }

            //去掉IE6~8中的标签项链接和按钮链接的虚线边框
            !+[1,] && $ele.find(">.btn a").add($links).attr("hidefocus", "true");
        ...
        if(seamless)
        {
            var prefixNums = scrollNums*(1 + appends),
                afterNums = cansee + appends + scrollNums - 1,
                $clonePrefix = $contents.slice(-prefixNums).each(function(index){ this.cloneIndex = index }).clone(true).addClass("clone"),
                $cloneAfter = $contents.slice(0, afterNums).each(function(index){ this.cloneIndex = index + amount + prefixNums }).clone(true).addClass("clone");
        ...
        switch(effect)
        {
            ...
            case "scrollx":
                if($contents.width() == jQuery(window).width())
                {
                    //这里针对100%宽度的幻灯片切换做对应处理
                    $contents.css("width", scrollBase);    
                    jQuery(window).resize(function()
                    {
                        scrollValue = scrollBase = jQuery(window).width();
            ...
            case "scrolly":
                $contentsParent.css({width: "auto"});
                $contents.show();
                direct = "scrollTop";
                break;

            case "accordition":
                acc_direct = $contents.css("float") === "none" ? "height" : "width";
                acc_value = $contents[0][acc_direct === "width" ? "scrollWidth" : "scrollHeight"];
                params.easing === undefined && (easing = acc_direct === "width" ? "easeOutQuad" : "easeOutCubic"); //手风琴效果的默认缓动算法
                $contents[acc_direct](accordition);
                break;
        }
        ...
        {
            //是否能够继续执行
            if(isLocked || (oldIndex === index && !doMust)) return;

            //相关变量声明
            var type = index >= oldIndex || (index === minIndex && oldIndex === pages) ? "next" : "prev", //确定是往前走，还是往后走
                _oldIndex = oldIndex,  //用于回调函数使用，确保在回调函数中的oldIndex参数指向滚动前显示内容项所在的索引位置
                endCallback = doAuto, //默认结束时的回调函数是执行自动播放的检测工作
                scrollDistance;
            ...
            //图片懒加载
            config.lazyload && doLazyload[index] === undefined && doLazyload(index, type);

            //切换之前先执行回调函数
            config.startFun && config.startFun.call({ele: $ele, contents: $contents, tags: $tags}, index, _oldIndex);

            //对切换之后执行的回调函数继续处理
            endFun && (endCallback = function()
            {
                endFun.call({ele: $ele, contents: $contents, tags: $tags}, index, _oldIndex);
                doAuto();
            });
            ...
            switch(effect)
            {
                //淡入淡出
                case "fade":
                    $contents.eq(oldIndex).fadeOut(duration, easing);
                    $contents.eq(index).fadeIn(duration, easing, endCallback);
                    break;

                //仅淡入效果
                case "fadeIn":
                    $contents.hide().slice(scrollNums * index, scrollNums * (index + 1)).fadeIn(duration, easing, endCallback);
                    break;

                //水平滚动
                case "scrollx":         
                    scrollDistance = !seamless && index === pages - 1 ? scrollMax : scrollValue * (index);
                    doStyle ? $contentsParent.stop().animate({left: -scrollDistance}, doMust ? 0 : duration, easing, endCallback) : 
                              $wrap.stop().animate({scrollLeft: scrollDistance}, doMust ? 0 : duration, easing, endCallback);
                    break;

                //垂直滚动
                case "scrolly":
                    scrollDistance = !seamless && index === pages - 1 ? scrollMax : scrollValue * (index);
                    $wrap.stop().animate({scrollTop: scrollDistance}, doMust ? 0 : duration, easing, endCallback);
                    break;

                //手风琴效果
                case "accordition":
                    $contents.eq(index).stop().animate(acc_direct === "width" ? {width: acc_value} : {height: acc_value}, duration, easing, endCallback);
                    $contents.not(":eq(" + index + ")").stop().animate(acc_direct === "width" ? {width: accordition} : {height: accordition}, duration, easing);
                    break;

                //通过类名控制状态显示
                case "widthClass":
                    $contents.removeClass("show").slice(scrollNums * index, scrollNums * (index + 1)).addClass("show");
                    endCallback !== undefined && endCallback();
                    break;
            ...
            //是否需要进行标签项的滚动操作
            tagScroll !== undefined && doTagScroll(oldIndex, index);

            //是否需要针对内容项的文本标题（class为“text”的文本内容）进行处理
            config.text !== "none" && doText(_oldIndex, index);
        ...
        //动作：自动切换
        function doAuto(delayValue)
        {
            if(isLocked)
            {
                return;
            }
            else if(config.doMarquee)
            {
                doMarquee();
            }
            else if(auto && !isPaused)
            {
                clearTimeout(stopAuto);
                stopAuto = setTimeout(function()
                {
                    doShow(oldIndex + 1);
                }, delayValue || config.delay);
            }
        }
        ...
        {
            isPaused = false;
            doAuto(500); //在暂停并重新启用自动后使用尽可能少的延时处理
        }
        ...
        //动作：针对内容项的文本标题（class为“text”的文本内容）进行处理
        function doText(oldIndex, index)
        {
            var $oldText = $text.eq(oldIndex),
                $curText = $text.eq(index);
            
            
            if(config.text === "slide")
            {
                $text.stop().hide();
                $oldText.show().slideUp(200, function()
                {
        ...
        //动作：标签项的滚动处理
        function doTagScroll(oldIndex, index)
        {
            var scrollPages = Math.floor(tagDistance / tagLength),
                scrollState = scrollPages + tagCansee,
                scrollDistance;

            //判断是否需要滚动
            if(oldIndex === index)
            {
                scrollDistance = 0; //首次显示默认
            }
            else if(oldIndex === 0 && index === pages - 1)
            {
                scrollDistance = (pages - tagCansee) * tagLength; //从第一页切换到最后一页
            }
            else if(oldIndex === pages - 1 && index === 0)
            {
                scrollDistance = 0; //从最后一页切换到第一页
            }
            else if(index >= scrollState - 1 && index !== pages - 1)
            {
                scrollDistance = (scrollPages + 1) * tagLength; //往后走
            }
            else if(index === scrollState - tagCansee && index !== 0)
            {
                scrollDistance = (scrollPages - 1) * tagLength; //往前走
            }
            
            //执行滚动
            if(scrollDistance !== undefined)
            {
                tagDistance = scrollDistance;
                tagScrollBox.stop().animate( tagScroll === "scrollLeft" ? {scrollLeft: scrollDistance} : {scrollTop: scrollDistance} );
            }
        }

        //动作：执行图片懒加载
        function doLazyload(index, type)
        {
            var start = scrollNums * index + (index === minIndex || type === "prev" ? 0 : cansee - scrollNums),
                end = scrollNums * (index + 1) + (type === "prev" ? 0 : cansee - scrollNums),
                $operater = $contents.slice(start, end);

            //将克隆的内容项加入到操作对象中
            seamless && ($operater = $operater.map(function(index, ele)
            {
                var cloneIndex = ele.cloneIndex;
                return cloneIndex !== undefined ? $contents.eq(cloneIndex)[0] : null;
            }).addBack());

            //执行操作
            $operater.find("[_src]").addBack("[_src]").each(function(index, ele)
            {
                var _src = ele.getAttribute("_src");
                ele.tagName.toLowerCase() === "img" ? ele.setAttribute("src", _src) : ele.style.backgroundImage = "url(" + _src + ")";
                ele.removeAttribute("_src");
            });
        ...
        jQuery.data(ele, "slide_doLock", doLock);
        jQuery.data(ele, "slide_doUnlock", doUnlock);
...
//通过调用.slide()方法实现选项卡切换
jQuery.fn.tab = function(params)
{
    //设置选项卡的必备参数
    params = params === undefined ? {} : params;
    params.keepTags = true;
    ...
    return this.each(function(index, ele)
    {
        var $ele = jQuery(ele),
            ...
            amount = $children.length;   //子节点个数

        //手风琴-无标题项的效果处理
        if(params.effect === "accordition" && params.accordition > 0)
        {
            $ele.slide(jQuery.extend({ tags: ">*", contents: ">*" }, params));
        }
        //结构：div{标题1,标题2,标题3}，div{内容1,内容2,内容3}
        else if($children.filter(".content").length === 1)
        {
            $ele.slide(params);
        }
        //结构：div{标题1, 内容1}, div{标题2, 内容2}, div{标题3, 内容3}
        else if($children.map(function(){ return this.children.length }).toArray().unique().join("") === "2" && $children.map(function(){ return this.nodeName }).toArray().unique().length === 1)
        {
            $ele.slide(jQuery.extend({ tags: ">*>:first-child", contents: ">*>:last-child" }, params));
        }
        //结构：div{标题1,内容1,标题2,内容2,标题3,内容3}
        else
        {
            $ele.slide(jQuery.extend({ tags: ">:nth-child(odd)", contents: ">:nth-child(even)" }, params));
        }
    });
};
...
Extend.getID = function(id)
{
    return jQuery(typeof id === "string" ? "#" + id : id);
};
Extend.tabChange = function(id, eventName, autoChange, delay)
{
    Extend.getID(id).tab({trigger: eventName === "onclick" ? "click" : "mouseenter" , auto: autoChange, delay: delay, effect: "widthClass" });
};
Extend.scrollWith = function(id)
{
    var $ele = Extend.getID(id);
    $ele.children().wrapAll("<div id='" + (id += "Inner") + "' style='width:" + $ele.width() + "px;'></div>").end();
    Extend.getID(id).scrollWith(); //注：id变量的值在上一语句中发生了变更
};
Extend.AddToFavorite = function()
{
    jQuery.browser.setHome();
};
Extend.SetHome = function()
{
    jQuery.browser.addFavorite();
};
Extend.setCookie = function(name, value, expires, path, domain, secure)
{
    jQuery.setCookie(name, value, expires, {path: path, domain: domain, secure: secure});
};
Extend.getCookie = function(name)
{
    jQuery.getCookie(name);
};
Extend.delCookie = function(name, path, domain)
{
    jQuery.delCookie(name, {path: path, domain: domain});
};
Extend.getCurrentStyle=function(id, styleName)
{
    return Extend.getID(id).css(styleName);
};
Extend.setStyle=function(id, styleName, value)
{
    Extend.getID(id).css("styleName", value);
};
