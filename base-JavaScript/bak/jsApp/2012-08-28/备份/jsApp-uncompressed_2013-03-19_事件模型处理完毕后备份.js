...
(function(window){
	...
	var 
		...
		//事件绑定快捷方式-事件名称集
		eventNames = "mousewheel,mouseover,mousemove,mouseout,mousedown,mouseup,click,dblclick,focus,blur,change,keydown,keypress,keyup,load,unload,beforeunload,resize,scroll,error,contextmenu,hashchange".split(","),
	...
	jsApp.fn = jsApp.prototype = {
		...
		{
			return mapcall(this, function(ele)
			{
	    ...
	    //兼容DOM3级规范中的mouseenter事件
		/*----------------------------------
		 *说明：该事件的定义同mouseover，但该事件不会发生冒泡，鼠标在子级元素的移动不会触发父级元素的该事件。
		 *=========
		 *【参数】
	     *1. handler: （类型：function，必备）需要绑定的处理程序
	     *2. capture: （类型：boolean，可选）是否进行事件捕捉（在IE的非标准事件模型中不支持事件捕捉！）
	     *--------------------------------------------------------------------------------------------------------------------*/
	    mouseenter: function(handler, capture)
	    {
	    	return mapcall(this, function(ele)
	    	{
	    		return jsApp.hover.call(this, "mouseenter", ele, handler, capture);
	    	});
	    },

	    //兼容DOM3级规范中的mouseleave事件
		/*----------------------------------
		 *说明：该事件的定义同mouseout，但该事件不会发生冒泡，鼠标在子级元素的移动不会触发父级元素的该事件。
		 *=========
		 *【参数】
	     *1. handler: （类型：function，必备）需要绑定的处理程序
	     *2. capture: （类型：boolean，可选）是否进行事件捕捉（在IE的非标准事件模型中不支持事件捕捉！）
	     *--------------------------------------------------------------------------------------------------------------------*/
	    mouseleave: function(handler, capture)
	    {
	    	return mapcall(this, function(ele)
	    	{
	    		return jsApp.hover.call(this, "mouseleave", ele, handler, capture);
	    	});
	    },
	...
	//使用jsApp.extend进行扩展的对象需要通过jsApp.xx来进行调用；而使用jsApp.fn.extend进行扩展的对象则需要通过new jsApp().xx来进行调用。
	...
	jsApp.extend({
		...
		//对mouseenter和mouseleave事件进行兼容处理
		hover: function(name, ele, handler, capture)
		{
			if(("on" + name) in ele)
    		{
	    		return this.bind(name, handler, capture);
    		}
    		return this.bind(name === "mouseenter" ? "mouseover" : "mouseout", function(e)
    		{
    			var target = e.target;
