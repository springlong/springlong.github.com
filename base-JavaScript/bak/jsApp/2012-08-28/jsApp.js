...
(function(window){
	...
	var
		...
		//另外有些操作需要针对匹配集合中的每一个元素都执行操作，这时通过该函数进行统一调配。
		/*----------------------------------
		 *参数说明：
		...
		map = function(match, callback)
		{
			...
			if(len)
			{
				...
				return callback.call(match, 0, match[0]); //确保返回匹配集合中第一个元素的操作结果
			}
			return match;
		},

		//用于元素获取时将nodeList转换为数组添加到jsApp的实例中
	...
	jsApp.fn = jsApp.prototype = {
		...
		//============================================================================================================================//
		//用于初始化jsApp对象实例
		//返回结果以类似数组的形式进行返回，可通过索引访问且拥有length属性，但这一切都是自定义的。
		init: function(selector)
		{
			var _this = this, ele, i, j, len, className;
			...
			if(jsApp.isString(selector))
			{
				...
				else if(/^[a-z]+$/i.test(selector) || selector === "*")
				{
					...
					makeArray.call(_this, ele);
				}
				//根据类名获取
				else if(/^.[\w-]+$/i.test(selector))
				{
					className = selector.substring(1, selector.length);
					...
					else
					{
						...
						for(i = 0, j = 0, len = ele.length; i < len; i++)
						{
				...
				else
				{
					...
					{
						document.styleSheets["jsApp_selectorStyle"].addRule(selector, "q:a");  //为指定选择器元素添加样式
		...
		//获取匹配集合中所有元素的第一个子元素所组成的数组，返回值类型：Array。
		/*----------------------------------
	...
	jsApp.extend = jsApp.fn.extend = function()
	{
		...
		if(name === true || jsApp.isObject(obj))
		{
	...
	jsApp.extend({
		...
		//回调函数传递两个参数：第一个表示索引位置；第二个表示当前索引位置的名称或值。
		each: function(obj, callback)
		{
			var name, i, len;

			if(jsApp.isObject(obj))
			{
				for(name in obj)
				{
		...
		//判断类型是否为：Object对象（就是指除内置对象和HTML对象外的自定义对象）
