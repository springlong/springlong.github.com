    ...
    jsApp.fn = jsApp.prototype = /** @lends jsApp.prototype */{
        ...
        text: function(text)
        {
            ...
            if(jsApp.isFunction(text))
            {
                return this.each(function(index, ele)
                {
                    text = text.call(ele, index, getText(ele));
                    jsApp(ele).empty();
        ...
        globalEval: function(doSometing)
        {
            if(jsApp.isValidString(doSometing))
            {
                try
                {
                    ...
                    //当JavaScript代码字符串含有语法错误时，由于使用了try-catch语句在非IE中将不会抛出异常，但由于IE使用了window.execScript将抛出异常
