...
jQuery.fn.slide = function(params)
{
    ...
    return this.each(function(index, ele)
    {
        ...
        function doShow(index, doMust)
        {
            ...
            //图片懒加载
            params.lazyload && doLazyload[index] === undefined && doLazyload(index, type);
            ...
            switch(effect)
            {
                //淡入淡出
                case "fade":
                    $contents.eq(oldIndex).fadeOut(duration);
                    $contents.eq(index).fadeIn(duration, endCallback);
                    break;

                //仅淡入效果
                case "fadeIn":
                    $contents.hide().slice(scrollNums * index, scrollNums * (index + 1)).fadeIn(duration, endCallback);
                    break;
